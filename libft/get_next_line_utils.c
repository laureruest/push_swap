...
/*   Updated: 2024/02/28 20:22:53 by lruiz-es         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static long long int	srch_nl(struct s_buffer *bf)
{
	long long int	cur;
...
static void	ins_part_ln_r(struct s_nl *ln, struct s_buffer *bf, int fd)
{
	long long int	idx;
...
static char	*e_ln_n(struct s_nl *ln, struct s_buffer *bf, long long	int nll_mrk)
{
	long long int	idx;

	ln->nw_ln = malloc(ln->o_size + 2 + nll_mrk - bf->idx);
	if (!ln->nw_ln)
		return (NULL);
	ln->nw_size = 0;
	idx = 0;
	...
	if (ln->o_ln)
		free(ln->o_ln);
	while (bf->idx <= nll_mrk)
		ln->nw_ln[ln->nw_size++] = bf->buf[bf->idx++];
	ln->nw_ln[ln->nw_size] = '\0';
	return (ln->nw_ln);
}

static char	*e_ln_n_n(struct s_nl *ln, struct s_buffer *bf)
{
	long long int	idx;
	long long int	limit;
